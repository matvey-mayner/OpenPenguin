...
local function average(colors)
	...
	return RGBToInteger(math.floor(averageRed / sColors), math.floor(averageGreen / sColors), math.floor(averageBlue / sColors))
end
...
local function to8Bit(color24Bit)
	local closestDelta, r, g, b, closestIndex, delta, openComputersPaletteR, openComputersPaletteG, openComputersPaletteB = math.huge, IntegerToRGB(color24Bit)
